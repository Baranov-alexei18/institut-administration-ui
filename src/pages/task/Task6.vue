...
import { useTeachersByLesson } from '@/composables/useTeachersByLesson';
...
const tableColumns: TableColumn[] = [
  ...
  { key: 'lessonTypeLabel', title: 'Тип занятия' },
...
const tableRows = computed<Record<string, string | number>[]>(() => {
  return teachers.value.map((teacher) => ({
    ...
    lessonTypeLabel: teacher.lessonTypeLabel,
    group: teacher.group,
...
<template>
  <section class="task">
    <header class="head">
      <h2 class="title">Преподаватели по занятиям</h2>
      <p class="desc">
        Получение перечня и общего числа преподавателей проводивших (проводящих) лекционные,
    ...
    <div class="filters">
      <AppSelect
        v-model="draftFilters.lessonType"
        label="Тип занятия"
