...
import { useExaminers } from '@/composables/useExaminers';
...
const tableRows = computed<Record<string, string | number>[]>(() => {
  return examiners.value.map((examiner) => ({
    id: examiner.id ?? '-',
    fullName: examiner.fullName,
    group: examiner.group,
    discipline: examiner.discipline,
...
<template>
  <section class="task">
    <header class="head">
      <h2 class="title">Экзаменаторы</h2>
      <p class="desc">
        Получение перечня преподавателей, принимающих (принимавших) экзамены в указанных группах,
        по указанным дисциплинам, в указанном семестре.
      </p>
    </header>

    <div class="filters">
      <AppSelect v-model="draftFilters.group" label="Группа" :options="groupOptions" />
      <AppSelect v-model="draftFilters.discipline" label="Дисциплина" :options="disciplineOptions" />
    ...
    <AppTable
      ...
      :rows="tableRows"
      empty-text="Экзаменаторы по выбранным фильтрам не найдены."
    />
  </section>
</template>
